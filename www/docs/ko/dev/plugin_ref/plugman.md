...
# Plugman을 사용하여 플러그인 관리 하기

버전 3.0 이후, Cordova는 플러그인으로 모든 기기 API를 구현하고 기본적으로 비활성화 시켜 놓습니다. 또한 다음 개요에서 여러분이 어떤 워크플로우를 선택하는지에 의존해 플러그인을 추가하고 제거하는 두 가지 방법을 지원합니다.

*   cross-platform 워크플로우를 사용하는 경우 플러그인을 추가하기 위해서 명령줄 인터페이스에 작성하는 방식으로 `cordova` CLI 유틸리티를 사용합니다. CLI는 지정된 플랫폼들을 위한 플러그인을 한 번에 수정합니다.

*   platform-centered 워크플로우를 사용하는 경우 각 타겟 플랫폼별로 lower-level [Plugman][1] 명령줄 인터페이스를 사용합니다. 

 [1]: https://github.com/apache/cordova-plugman/

이 섹션은 Plugman 유틸리티를 자세히 설명합니다. 소스코드를 수정하거나 node module로서 Plugman 사용에 관한 더 많은 정보를 원하시면  [README 파일][2] 을 참조 하십시오..

 [2]: https://github.com/apache/cordova-plugman/blob/master/README.md

## Plugman 설치

Plugman를 설치 하려면 [노드][3] 가 컴퓨터에 설치 되어 있어야 합니다. 그럼 어느 디렉토리에서나 이용 가능하도록 글로벌 옵션으로 plugman을 설치하기 위한 다음 명령을 실행할 수 있습니다. 
...
`git`또한 여러분이 원하는 `PATH`에 설치되어 있어야 원격 git Url에서 바로 플러그인을 설치할 수 있습니다.
